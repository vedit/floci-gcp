...
public class GcsMultipartService {
    ...
    @Inject
    public GcsMultipartService(GcsService gcs) {
        this.gcs = gcs;
        uploads = gcs.multipartUploads();
    }
    public GcsMultipartUpload initiate(String bucket, String object, String type, Map<String,String> metadata) {
    ...
    public GcsMultipartUpload get(String bucket, String object, String id) {
        return gcs.withMultipartBucketLock(bucket, () -> {
            gcs.getBucket(bucket);
    ...
    public GcsMultipartUpload.Part putPart(String bucket, String object, String id, int number, byte[] bytes, String contentMd5) {
        return gcs.withMultipartBucketLock(bucket, () -> {
            ...
            GcsMultipartUpload.Part part = new GcsMultipartUpload.Part(number, "\"" + HexFormat.of().formatHex(digest) + "\"", Instant.now().toString(), bytes.clone());
    ...
    public void abort(String bucket, String object, String id) {
        gcs.withMultipartBucketLock(bucket, () -> {
            get(bucket, object, id); uploads.delete(id); uploads.checkpoint();
            return null;
        });
    }
    public List<GcsMultipartUpload> list(String bucket, String prefix) {
        return gcs.withMultipartBucketLock(bucket, () -> {
            gcs.getBucket(bucket);
    ...
    public List<GcsMultipartUpload.Part> listParts(String bucket, String object, String id, int marker) {
        return gcs.withMultipartBucketLock(bucket, () -> get(bucket, object, id).parts.values().stream()
                .filter(part -> part.number() > marker)
                .sorted(Comparator.comparingInt(GcsMultipartUpload.Part::number)).toList());
    }
